 ...
 *        translating user input into board manipulation. Implements Constants
 *        to ensure consistent char usage.
 */
public abstract class Player implements Constants {
	/**
	 * Boolean to determine if player can make a move or not.
	 */
	...
	protected BufferedReader socketIn;// input from client
	 ...
	 */
	protected Player(String name, char letter, Socket socket) {
		this.name = name;
		this.letter = letter;
		try {
	...
	protected void setSuspended(boolean b) {
		suspended = b; // set value of suspended
	}
}
